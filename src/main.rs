...
impl ChromeDriverManager {
    fn new(port: u16, debug: bool) -> Self {
        ChromeDriverManager {
            process: None,
            port,
            debug,
...
async fn main() -> Result<()> {
    ...
    let mut chromedriver = ChromeDriverManager::new(chromedriver_port, args.debug);
    chromedriver.start()?;
    ...
    navigate_and_wait(&driver, url, Duration::from_secs(args.wait), is_piped, args.debug).await?;
    
    // Execute JavaScript if provided
    if let Some(js_code) = &args.js {
        execute_javascript(&driver, js_code).await?;
    }
    
    // Capture screenshot or recording
    if is_recording {
        create_recording(&driver, recording_length, &output_path, is_piped, args.debug).await?;
    } else {
        take_screenshot(&driver, &output_path, is_piped, args.debug).await?;
    }
